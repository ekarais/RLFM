...
def set_hyperparameters(agents, alpha=0.1, gamma=0.1, epsilon=0.1):
    '''
    Sets Hyperparameters for learning agents.
    '''
    for agent in agents:
...
def get_agents_equal(sellr_reserve = 20, buyer_reserve = 100,
    ...
    for i in range(n_rnd_int_buyers):
        rnd_int_buyers.append(RandInterBuyer(agent_id = f'B[i{i}]', reservation_price = buyer_reserve, default_price = buyer_default))
    #RandomIntervalSellers    
    for i in range(n_rnd_int_sellrs):
        rnd_int_sellrs.append(RandInterSeller(agent_id = f'S[i{i}]', reservation_price = sellr_reserve, default_price = sellr_default))
    
    #Q Learning Buyers
    for i in range(n_q_learn_buyers):
        q_learn_buyers.append(QLearningBuyer(agent_id = f'B[q{i}]', reservation_price = buyer_reserve, default_price = buyer_default, n_states=n_states))
    #Q Learning Sellers
    for i in range(n_q_learn_sellrs):
        q_learn_sellrs.append(QLearningSeller(agent_id = f'S[q{i}]', reservation_price = sellr_reserve, default_price = sellr_default, n_states=n_states))
